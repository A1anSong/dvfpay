...
// 更新行
const updateTransactionFunc = async(row) => {
  const res = await findTransaction({ ID: row.ID })
  type.value = 'update'
  if (res.code === 0) {
    formData.value = res.data.retransaction
    dialogFormVisible.value = true
  }
}

// 删除行
const deleteTransactionFunc = async(row) => {
  const res = await deleteTransaction({ ID: row.ID })
  if (res.code === 0) {
    ElMessage({
      type: 'success',
      message: '删除成功',
