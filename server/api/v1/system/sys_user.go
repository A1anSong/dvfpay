...
// @Produce application/json
// @Param data body request.PageInfo true "页码, 每页大小"
// @Success 200 {object} response.Response{data=response.PageResult,msg=string} "分页获取商户列表,返回包括列表,总数,页码,每页数量"
// @Router /user/getMerchantList [post]
func (b *BaseApi) GetMerchantInfoList(c *gin.Context) {
	var pageInfo request.PageInfo
	_ = c.ShouldBindJSON(&pageInfo)
	if err := utils.Verify(pageInfo, utils.PageInfoVerify); err != nil {
		response.FailWithMessage(err.Error(), c)
	...
	if err, list, total := userService.GetMerchantInfoList(pageInfo); err != nil {
		global.GVA_LOG.Error("获取失败!", zap.Error(err))
		response.FailWithMessage("获取失败", c)
	} else {
		response.OkWithDetailed(response.PageResult{
...
// @Success 200 {object} response.Response{data=response.PageResult,msg=string} "分页获取商户列表,返回包括列表,总数,页码,每页数量"
// @Router /user/getMerchantList [post]
func (b *BaseApi) GetMerchantList(c *gin.Context) {
	if err, list := userService.GetMerchantList(); err != nil {
		global.GVA_LOG.Error("获取失败!", zap.Error(err))
		response.FailWithMessage("获取失败", c)
	} else {
		response.OkWithDetailed(response.PageResult{
