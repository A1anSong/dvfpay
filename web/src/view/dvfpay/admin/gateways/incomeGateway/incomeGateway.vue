        ...
        <el-table-column align="left" label="按钮组">
          <template #default="scope">
            <el-button
              ...
              size="small"
              class="table-button"
              @click="updateIncomeGatewayFunc(scope.row)"
            >变更
            </el-button>
...
import {
  createIncomeGateway,
  deleteIncomeGateway,
  deleteIncomeGatewayByIds,
  updateIncomeGateway,
  findIncomeGateway,
  getIncomeGatewayList,
} from '@/api/dvfpay/incomeGateway'
...
const getTableData = async() => {
  const table = await getIncomeGatewayList({ page: page.value, pageSize: pageSize.value, ...searchInfo.value })
  if (table.code === 0) {
...
const onDelete = async() => {
  ...
  const res = await deleteIncomeGatewayByIds({ ids })
  if (res.code === 0) {
    ElMessage({
      type: 'success',
      message: '删除成功',
...
// 更新行
const updateIncomeGatewayFunc = async(row) => {
  const res = await findIncomeGateway({ ID: row.ID })
  type.value = 'update'
  if (res.code === 0) {
    formData.value = res.data.reincomeGateway
    dialogFormVisible.value = true
  }
}

// 删除行
const deleteIncomeGatewayFunc = async(row) => {
  const res = await deleteIncomeGateway({ ID: row.ID })
  if (res.code === 0) {
    ElMessage({
      type: 'success',
      message: '删除成功',
...
const enterDialog = async() => {
  ...
  switch (type.value) {
    case 'create':
      res = await createIncomeGateway(formData.value)
      break
    case 'update':
      res = await updateIncomeGateway(formData.value)
      break
    default:
      res = await createIncomeGateway(formData.value)
      break
