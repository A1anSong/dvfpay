...
import { getPayoutGatewayList } from '@/api/dvfpay/payoutGateway'
...
const payoutGatewayOptions = ref([])
const merchantOptions = ref([])

const setPayoutGatewayOptions = async() => {
  payoutGatewayOptions.value = []
  const res = await getPayoutGatewayList({ page: 1, pageSize: 999 })
  res.data.list && res.data.list.forEach(item => {
    const option = {
      ID: item.ID,
      name: item.name,
    }
    payoutGatewayOptions.value.push(option)
  })
}

setPayoutGatewayOptions()
...
const setMerchantIds = () => {
  tableData.value && tableData.value.forEach((payoutGatewayAuth) => {
    const merchantIds = payoutGatewayAuth.merchants && payoutGatewayAuth.merchants.map(i => {
      return i.ID
    })
    payoutGatewayAuth.merchantIds = merchantIds
  })
}
